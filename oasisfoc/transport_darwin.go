...
// run-loop thread (the standard hidapi-on-macOS pattern), queued for focoasis_read to
// pop with a timeout.
...
static void focoasis_input_cb(void* ctx, IOReturn r, void* sender, IOHIDReportType type,
                           uint32_t reportID, uint8_t* report, CFIndex len) {
    focoasis_dev* d = (focoasis_dev*)ctx;
...
static void* focoasis_run(void* arg) {
    focoasis_dev* d = (focoasis_dev*)arg;
    CFRunLoopRef rl = CFRunLoopGetCurrent();
    IOHIDDeviceScheduleWithRunLoop(d->dev, rl, kCFRunLoopDefaultMode);
    IOHIDDeviceRegisterInputReportCallback(d->dev, d->inbuf, OASIS_RPT, focoasis_input_cb, d);
    while (!d->stop) {
...
// focoasis_open finds a HID device with VID/PID (and locationID if wantLoc != 0),
// opens it, starts the reader thread, and returns the context (or NULL).
static focoasis_dev* focoasis_open(uint32_t vid, uint32_t pid, uint32_t wantLoc, uint32_t* outLoc) {
    ...
    focoasis_dev* d = (focoasis_dev*)calloc(1, sizeof(focoasis_dev));
    d->dev = chosen;
    pthread_mutex_init(&d->mu, NULL);
    pthread_cond_init(&d->cond, NULL);
    if (pthread_create(&d->thread, NULL, focoasis_run, d) == 0) d->started = 1;
    if (outLoc) *outLoc = chosenLoc;
    // mgr intentionally retained so its matching stays alive.
    return d;
}

// focoasis_enum lists matching devices' locationIDs into outLoc (up to maxN). Returns count.
static int focoasis_enum(uint32_t vid, uint32_t pid, uint32_t* outLoc, int maxN) {
    IOHIDManagerRef mgr = IOHIDManagerCreate(kCFAllocatorDefault, kIOHIDOptionsTypeNone);
    if (!mgr) return -1;
    IOHIDManagerSetDeviceMatching(mgr, NULL);
    IOHIDManagerOpen(mgr, kIOHIDOptionsTypeNone);
...
// focoasis_write sends one interrupt OUT report. buf[0] is the report ID (0x00); the
// remaining len-1 bytes are the report payload (mirrors hidapi hid_write).
static int focoasis_write(focoasis_dev* d, uint8_t* buf, int len) {
    if (len < 1) return -1;
...
// focoasis_read pops one queued input report into buf, waiting up to timeoutMS
// (<=0 = non-blocking). Returns bytes copied, or 0 if none/timed out.
static int focoasis_read(focoasis_dev* d, uint8_t* buf, int cap, int timeoutMS) {
    pthread_mutex_lock(&d->mu);
...
static void focoasis_close(focoasis_dev* d) {
    if (!d) return;
    if (d->started) {
        d->stop = 1;
...
type iokitTransport struct{ d *C.focoasis_dev }

// Enumerate lists all attached Oasis focuser HID devices by USB locationID.
func Enumerate() ([]DeviceInfo, error) {
	const maxN = 16
	var locs [maxN]C.uint32_t
	n := int(C.focoasis_enum(C.uint32_t(VID), C.uint32_t(PID), &locs[0], maxN))
	if n < 0 {
...
func openDev(wantLoc uint32) (Transport, DeviceInfo, error) {
	var loc C.uint32_t
	d := C.focoasis_open(C.uint32_t(VID), C.uint32_t(PID), C.uint32_t(wantLoc), &loc)
	if d == nil {
		return nil, DeviceInfo{}, errors.New("no Oasis focuser (none attached, held by another process, or HID access denied)")
	}
...
// OpenLocation opens the Oasis focuser at a specific USB locationID (from Enumerate).
...
func (t *iokitTransport) Write(buf []byte) (int, error) {
	...
	n := int(C.focoasis_write(t.d, (*C.uint8_t)(unsafe.Pointer(&buf[0])), C.int(len(buf))))
	if n < 0 {
...
func (t *iokitTransport) Read(buf []byte, timeoutMS int) (int, error) {
	...
	n := int(C.focoasis_read(t.d, (*C.uint8_t)(unsafe.Pointer(&buf[0])), C.int(len(buf)), C.int(timeoutMS)))
	return n, nil
}

func (t *iokitTransport) Close() error {
	if t.d != nil {
		C.focoasis_close(t.d)
		t.d = nil
	}
	return nil
}
