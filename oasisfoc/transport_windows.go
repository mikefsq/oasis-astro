...
// Windows HID transport for the Oasis focuser: pure Go over SetupAPI (enumeration)
...
// Enumerate lists all attached Oasis focuser HID devices.
...
func openFirst() (Transport, DeviceInfo, error) {
	...
	if len(devs) == 0 {
		return nil, DeviceInfo{}, errors.New("no Oasis focuser found")
	}
	return openWindows(devs[0])
}

// OpenLocation opens the Oasis focuser whose path hashes to loc (from Enumerate).
func OpenLocation(loc uint32) (Transport, DeviceInfo, error) {
	...
	return nil, DeviceInfo{}, fmt.Errorf("no Oasis focuser at location 0x%08x", loc)
}
