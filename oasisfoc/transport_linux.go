...
// Linux HID transport for the Oasis focuser: pure Go over hidraw (/dev/hidrawN).
// The Oasis focuser uses interrupt endpoints, so commands/replies are ordinary
...
// Enumerate lists all attached Oasis focuser hidraw devices.
...
func openFirst() (Transport, DeviceInfo, error) {
	...
	return nil, DeviceInfo{}, errors.New("no Oasis focuser found in /sys/class/hidraw")
}

// OpenLocation opens the Oasis focuser at a specific hidraw location (from Enumerate).
func OpenLocation(loc uint32) (Transport, DeviceInfo, error) {
	...
	return nil, DeviceInfo{}, fmt.Errorf("no Oasis focuser at hidraw location %d", loc)
}
